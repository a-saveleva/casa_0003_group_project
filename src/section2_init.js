...
// ✅ 构造 recs 数据（在 secondMap.on('load') 内）
...
setupScrollHandler(secondMap);  // ✅ 同步第一幕图层（轨道、车站等）
 ...
 function drawRadialChart(props) {
  ...
  function handleHover(e){
    ...
    ['30min','60min','90min','120min'].forEach(dur=>{
      ...
      secondMap.setFilter(`iso-fill-${dur}`, ['match',['get','id'],showIds,true,false]);
      secondMap.setFilter(`iso-line-${dur}`, ['match',['get','id'],showIds,true,false]);
    });
  }
